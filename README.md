...
- [Building-Kafka-on-Linux](#building-kafka-on-linux)
  * [第一章 Linux系统(centOS7)上Kafka的搭建和使用教程（超全）](#----linux---centos7--kafka------------)
    + [一、安装JDK](#----jdk)
      - [1.查看系统版本 cat /etc/redhat-release](#1-------cat--etc-redhat-release)
      - [2.安装之前先查看一下有无系统自带jdk](#2---------------jdk)
      - [3.如果有就使用批量卸载命令](#3------------)
      - [4.直接yum安装1.8.0版本openjdk](#4--yum--180--openjdk)
      - [5.检查版本](#5----)
      - [6.生效配置（三选一，按照自己当前需求选择）](#6--------------------)
      - [7.查看变量](#7----)
    + [二、安装Kafka](#----kafka)
      - [1.利用wget下载Kafka压缩包](#1--wget--kafka---)
      - [2.解压](#2--)
      - [3.命名化简（为了简化以后的shell命令和脚本）](#3------------shell------)
    + [三、安装ZooKeeper服务](#----zookeeper--)
      - [1.方案一 使用kafka自带的zookeeper](#1------kafka---zookeeper)
      - [2.方案二 独立安装zookeeper服务](#2--------zookeeper--)
        * [（1）下载](#-1---)
        * [（2）解压安装并简化命名（与Kafka安装流程类似）](#-2------------kafka-------)
        * [（3）创建配置文件](#-3-------)
      - [3.方案三 使用hadoop集群cdh等套件中的zookeeper（目前探索中，未完待续）](#3------hadoop--cdh-----zookeeper------------)
    + [四、kafka操作](#--kafka--)
      - [1.方案一：使用kafka自带的zookeeper](#1------kafka---zookeeper)
        * [(1)进入Kafka目录](#-1---kafka--)
        * [(2)首先启动kafka自带的zookeeper](#-2-----kafka---zookeeper)
        * [(3)接下来启动kafka](#-3------kafka)
        * [(4)使用Kafka](#-4---kafka)
        * [(5)结束之后，停止kafka服务器](#-5--------kafka---)
        * [(6)停止zookeeper](#-6---zookeeper)
      - [2.方案二：独立安装zookeeper服务](#2--------zookeeper--)
        * [（1）启动ZooKeeper服务器](#-1---zookeeper---)
        * [(2)在新的终端里，按照方案一中的（3）（4）（5）执行](#-2----------------3--4--5---)
        * [(3)停止ZooKeeper服务器](#-3---zookeeper---)
      - [3.方案三、目前正在探索中](#3-----------)
    + [附：前台执行和后台执行方法对比](#---------------)
